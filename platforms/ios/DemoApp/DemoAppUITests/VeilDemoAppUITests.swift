...
class VeilDemoAppUITests: XCTestCase {
    ...
    func testVeil() {
        ...
        moreButton?.tap()
        let statusLabel = app!.staticTexts.element(matching: .staticText, identifier: "veil.test.statusLabel")

        let wait = FUIWait(timeout: 10)
        wait.waitForExpressionTrue { () -> Bool in
